...
*/
 ...
 int txValue = 0, mode = 0, temperature = 20, speed = 5, direct = 0;  
 int oldtemperature = 0; //保存上次的模式  
    ...
    //  static bool oldopennn = true;
     ...
     if (deviceConnected)
     {
         if(opennn)
         {
             ...
             if ((oldMode != current_mode) || (oldtemperature != temperature) || (oldspeed != current_speed) || (olddirect != current_direct) || (!oldopennn && opennn))
             {
                 oled.clear();
                 oled.display();
